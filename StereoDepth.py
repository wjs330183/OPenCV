...
import camera_configs as camera_params
import open3d

camL = cv2.VideoCapture(0)  # 摄像头的ID不同设备上可能不同
camR = cv2.VideoCapture(2)  # 摄像头的ID不同设备上可能不同
...
class StereoDepth(object):
    ...
    def __init__(self, stereo_file, width=640, height=480, filter=True, use_open3d=True, use_pcl=True):
        ...
        if self.use_pcl:
            # 使用open3d显示点云
            self.pcl_viewer = open3d.PCLCloudViewer()
        if self.use_open3d:
            # 使用PCL显示点云
            self.open3d_viewer = open3d.Open3DVisual(camera_intrinsic=self.camera_config["K1"],
                                                     depth_width=width,
                                                     depth_height=height)
    ...
    def capture1(self, video):
        ...
        """
        cap = camL
        width, height, numFrames, fps = 1280, 480, 1, 60
        self.count = 0
        while True:
    ...
    def show_2dimage(self, frameL, frameR, points_3d, dispL, waitKey=0):
        ...
        """
        x, y, depth = cv2.split(points_3d)  # depth = points_3d[:, :, 2]
        depth_colormap = open3d.get_visual_depth(depth)
        dispL_colormap = open3d.get_visual_disparity(dispL)
        open3d.addMouseCallback("left", depth, info="depth=%fmm")
        open3d.addMouseCallback("right", depth, info="depth=%fmm")
        open3d.addMouseCallback("disparity-color", depth, info="depth=%fmm")
        open3d.addMouseCallback("depth-color", depth, info="depth=%fmm")
